 ...
 * ║   ╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚═════╝    ╚═╝                       ║
 * ║                                                                  ║
 * ║            HEADY MANAGER — THE BRAIN OF THE SYSTEM              ║
 * ║                                                                  ║
 * ║   ✦ Sacred Geometry v4.0 — φ (1.618) Governs All ✦             ║
 * ║   ◈ Built with Love by Heady™ — HeadySystems Inc.              ║
 * ║   ◈ Founder: Eric Haywood — 51 Provisional Patents              ║
 * ║   ◈ 18 Engines · 4 Foundation Packages · Liquid Architecture    ║
 * ║                                                                  ║
...
// FIX: Render healthCheckPath alias — Render expects /api/brain/health
...
// ═══════════════════════════════════════════════════════════════════════
// ─── WIRED ENGINES (Previously Dead — Fixed 2026-03-13) ─────────────
// ═══════════════════════════════════════════════════════════════════════
...
try {
  ...
  beeFactory = typeof BeeFactory === "function" ? new BeeFactory() : BeeFactory;
  logger.info("  ∞ HeadyBee Factory: LOADED (agents/bee-factory.js)");
} catch (err) {
  logger.warn(`  ⚠ HeadyBee Factory not loaded: ${err.message}`);
}

// ─── Hive Coordinator ───────────────────────────────────────────────
let hiveCoordinator = null;
try {
  const HiveCoordinator = require("./agents/hive-coordinator");
  hiveCoordinator = typeof HiveCoordinator === "function" ? new HiveCoordinator({ beeFactory }) : HiveCoordinator;
  if (hiveCoordinator && typeof hiveCoordinator.init === "function") {
    hiveCoordinator.init({ beeFactory, taskScheduler, patternEngine });
  }
  logger.info("  ∞ Hive Coordinator: LOADED (agents/hive-coordinator.js)");
} catch (err) {
  logger.warn(`  ⚠ Hive Coordinator not loaded: ${err.message}`);
}

// ─── Latent Space (Vector Operations) ───────────────────────────────
let latentSpace = null;
try {
  const LatentSpace = require("./src/hc_latent_space");
  latentSpace = typeof LatentSpace === "function" ? new LatentSpace({
    pineconeApiKey: process.env.PINECONE_API_KEY,
    dimensions: 768,
  }) : LatentSpace;
  if (latentSpace && typeof latentSpace.registerRoutes === "function") {
    latentSpace.registerRoutes(app);
  }
  // Expose latent space routes
  app.get("/api/latent/status", (req, res) => {
    res.json({
      ok: true,
      engine: "hc_latent_space",
      connected: !!latentSpace,
      dimensions: 768,
      ts: new Date().toISOString(),
    });
  });
  logger.info("  ∞ Latent Space: LOADED (src/hc_latent_space.js) — Vector routing active");
} catch (err) {
  logger.warn(`  ⚠ Latent Space not loaded: ${err.message}`);
}

// ─── Orchestrator (Task Routing) ────────────────────────────────────
let orchestrator = null;
try {
  const Orchestrator = require("./src/hc_orchestrator");
  orchestrator = typeof Orchestrator === "function" ? new Orchestrator({
    latentSpace,
    taskScheduler,
    patternEngine,
    beeFactory,
  ...
  if (orchestrator && typeof orchestrator.registerRoutes === "function") {
  ...
  app.get("/api/orchestrator/status", (req, res) => {
    res.json({
      ok: true,
      engine: "hc_orchestrator",
      connected: !!orchestrator,
      latentSpaceConnected: !!latentSpace,
      beeFactoryConnected: !!beeFactory,
      ts: new Date().toISOString(),
    });
  });
  logger.info("  ∞ Orchestrator: LOADED (src/hc_orchestrator.js) — Semantic routing active");
} catch (err) {
  logger.warn(`  ⚠ Orchestrator not loaded: ${err.message}`);
}

// ─── Conductor (AI Brain / Planner) ─────────────────────────────────
let conductor = null;
try {
  const Conductor = require("./src/hc_conductor");
  conductor = typeof Conductor === "function" ? new Conductor({
    orchestrator,
    latentSpace,
    pipeline,
    mcPlanScheduler,
  ...
  if (conductor && typeof conductor.registerRoutes === "function") {
    conductor.registerRoutes(app);
  }
  // Expose conductor routes
  app.get("/api/conductor/status", (req, res) => {
    res.json({
      ok: true,
      engine: "hc_conductor",
      connected: !!conductor,
      orchestratorConnected: !!orchestrator,
      pipelineConnected: !!pipeline,
      ts: new Date().toISOString(),
    });
  });
  logger.info("  ∞ Conductor: LOADED (src/hc_conductor.js) — AI coordination active");
} catch (err) {
  logger.warn(`  ⚠ Conductor not loaded: ${err.message}`);
}

// ─── Colab Runtime Manager (4x Pro+ Integration) ────────────────────
let colabRuntimeManager = null;
try {
  const ColabRuntimeManager = require("./services/colab-runtime-manager");
  colabRuntimeManager = typeof ColabRuntimeManager === "function" ? new ColabRuntimeManager({
    runtimes: 4,
    specializations: ["embedder", "evaluator", "learner", "general"],
    orchestrator,
    latentSpace,
  }) : ColabRuntimeManager;
  if (colabRuntimeManager && typeof colabRuntimeManager.registerRoutes === "function") {
    colabRuntimeManager.registerRoutes(app);
  }
  // Expose colab runtime routes
  app.get("/api/colab/status", (req, res) => {
    res.json({
      ok: true,
      engine: "colab-runtime-manager",
      connected: !!colabRuntimeManager,
      runtimes: 4,
      specializations: ["embedder", "evaluator", "learner", "general"],
      ts: new Date().toISOString(),
    });
  });
  logger.info("  ∞ Colab Runtime Manager: LOADED (services/colab-runtime-manager.js) — 4 runtimes");
} catch (err) {
...
try {
  ...
  if (pipeline && orchestrator && typeof pipeline.bind === "function") {
    pipeline.bind({
      orchestrator: orchestrator || null,
      conductor: conductor || null,
      latentSpace: latentSpace || null,
      colabRuntime: colabRuntimeManager || null,
    });
    logger.info("  ∞ Pipeline re-bound with Orchestrator + Conductor + LatentSpace + Colab");
  }

  // Wire hive coordinator events into pattern engine
  if (hiveCoordinator && patternEngine && typeof hiveCoordinator.on === "function") {
    hiveCoordinator.on("bee:completed", (data) => {
      patternEngine.observeSuccess(`bee:${data.type || "task"}`, data.durationMs || 0, {
        beeId: data.beeId, tags: ["headybee", "swarm"],
      });
    });
    hiveCoordinator.on("bee:failed", (data) => {
      patternEngine.observeError(`bee:${data.type || "task"}`, data.error || "unknown", {
        beeId: data.beeId, tags: ["headybee", "swarm", "failure"],
      });
    });
    logger.info("  ∞ Hive events → Pattern Engine wired");
  }

  // Wire conductor into self-critique for AI planning feedback
  if (conductor && selfCritiqueEngine && typeof conductor.on === "function") {
    conductor.on("plan:executed", (result) => {
      selfCritiqueEngine.recordCritique({
        context: `conductor:${result.planId || "plan"}`,
        weaknesses: result.weaknesses || [],
        severity: result.success ? "low" : "high",
        suggestedImprovements: result.improvements || [],
      });
    });
    logger.info("  ∞ Conductor → Self-Critique wired");
  }

  // Wire story driver to new engines
  if (storyDriver) {
    if (orchestrator && typeof orchestrator.on === "function") {
      orchestrator.on("task:routed", (data) => {
        storyDriver.ingestSystemEvent({
          type: "TASK_ROUTED",
          refs: { taskId: data.taskId, target: data.target, similarity: data.similarity },
          source: "orchestrator",
        });
      });
    }
    if (colabRuntimeManager && typeof colabRuntimeManager.on === "function") {
      colabRuntimeManager.on("runtime:assigned", (data) => {
        storyDriver.ingestSystemEvent({
          type: "COLAB_RUNTIME_ASSIGNED",
          refs: { runtimeId: data.runtimeId, specialization: data.specialization },
          source: "colab_runtime_manager",
        });
      });
    }
    logger.info("  ∞ Story Driver wired to Orchestrator + Colab events");
  }
} catch (err) {
  logger.warn(`  ⚠ Cross-engine wiring partial: ${err.message}`);
}

// ─── Master Status Endpoint (All Engines) ───────────────────────────
app.get("/api/system/engines", (req, res) => {
  res.json({
    ok: true,
    engines: {
      pipeline: { loaded: !!pipeline, module: "src/hc_pipeline.js" },
      monteCarlo: { loaded: !!mcPlanScheduler, module: "src/hc_monte_carlo.js" },
      patternEngine: { loaded: !!patternEngine, module: "src/hc_pattern_engine.js" },
      selfCritique: { loaded: !!selfCritiqueEngine, module: "src/hc_self_critique.js" },
      storyDriver: { loaded: !!storyDriver, module: "src/hc_story_driver.js" },
      taskScheduler: { loaded: !!taskScheduler, module: "src/hc_task_scheduler.js" },
      resourceManager: { loaded: !!resourceManager, module: "src/hc_resource_manager.js" },
      resourceDiagnostics: { loaded: !!resourceDiagnostics, module: "src/hc_resource_diagnostics.js" },
      beeFactory: { loaded: !!beeFactory, module: "agents/bee-factory.js" },
      hiveCoordinator: { loaded: !!hiveCoordinator, module: "agents/hive-coordinator.js" },
      latentSpace: { loaded: !!latentSpace, module: "src/hc_latent_space.js" },
