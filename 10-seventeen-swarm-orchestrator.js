...
class Swarm {
  ...
  submit(task) {
    ...
    if (this._queue.length >= this._maxQueue) {      // Queue is full
...
class ConsensusManager {
  ...
  _tryResolve(proposalId) {
    ...
    const tally = {};
    let totalWeight = 0;
    for (const [swarm, { decision, weight }] of proposal.votes) {      const effectiveWeight = weight * PHI;
